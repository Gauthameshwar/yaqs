...
"""Benchmark: transverse-field XXZ (Pauli) chain — unitary ensemble autocorrelator vs ED.

Hamiltonian (same convention as ``MPO.hamiltonian`` / Pauli matrices)::

    H = -J \\sum_i (X_i X_{i+1} + Y_i Y_{i+1}) - \\Delta \\sum_i Z_i Z_{i+1} - h \\sum_i X_i .

- **XXZ**: in-plane exchange ``J`` for ``XX`` and ``YY``, anisotropy ``\\Delta`` on ``ZZ``.
- **XXX**: set ``\\Delta = J`` (isotropic exchange).

Onsite autocorrelator::

    C_i(t) = \\langle Z_i(t) Z_i(0) \\rangle

Parallel YAQS (``unitary_ensemble_member_worker``) is compared to QuSpin exact
diagonalization on the **same** deep-copied initial MPS ensemble.

Environment variables (same timing knobs as the TFI benchmark where noted)::

    YAQS_J          in-plane coupling J (default 1.0)
    YAQS_DELTA      ZZ coupling \\Delta (default 1.0; set equal to J for XXX)
    YAQS_FIELD      transverse field h on -\\sum_i X_i (default 0.5)
    YAQS_NUM_STATES, YAQS_T_FINAL, YAQS_DT_SWEEP
    YAQS_SERIAL_FULL, YAQS_SERIAL_BENCH_STATES

Run::

    uv run python tests/analog/bench_xxz_transverse_unitary_ensemble_autocorrelator.py
"""
...
def _build_quspin_xxz_transverse(length: int, j_xy: float, delta: float, field: float):
    """Build H matching YAQS: -J(XX+YY) - Δ ZZ - h X (open chain)."""
    ...
    xx_terms = [[-j_xy, i, i + 1] for i in bonds]
    yy_terms = [[-j_xy, i, i + 1] for i in bonds]
    zz_terms = [[-delta, i, i + 1] for i in bonds]
    x_terms = [[-field, i] for i in range(length)]
    static = [["xx", xx_terms], ["yy", yy_terms], ["zz", zz_terms], ["x", x_terms]]
...
def _exact_autocorr_quspin(
    ...
    """Ensemble mean/stderr of C(t)=<Z_i(t)Z_i(0)> via QuSpin ED."""
    h_qs, basis = _build_quspin_xxz_transverse(length, j_xy, delta, field)
...
def _model_label(j_xy: float, delta: float) -> str:
    if abs(j_xy - delta) < 1e-12:
        return "XXX + transverse field"
    return "XXZ + transverse field"
...
def _save_plot(
    ...
    *,
    ...
    try:
        ...
        plt.title(f"{model_label} at site i={center_site} (N={num_states} random MPS)")
        plt.grid(alpha=0.3)
        plt.legend()
        plt.tight_layout()
...
def _save_combined_autocorr_plot(
    ...
    *,
    ...
    try:
        ...
        plt.title(f"{model_label} — site i={center_site} (N={num_states} random MPS)")
        plt.grid(alpha=0.3)
        plt.legend()
        plt.tight_layout()
        plt.savefig(output_path, dpi=150)
...
def _save_error_vs_ed_plot(
    ...
    *,
    ...
    try:
        ...
        plt.title(f"{model_label} — YAQS autocorrelator absolute error vs ED")
        plt.yscale("log")
        plt.grid(alpha=0.3)
        plt.legend()
        plt.tight_layout()
...
def main() -> None:
    ...
    model_label = _model_label(j_xy, delta)
    ...
    print(
        "Benchmark setup (Pauli convention): "
        f"L={length}, J_xy={j_xy}, Delta={delta}, h={field}, model={model_label}, "
        f"states={num_states}, t_final={t_final}, dt_sweep={dt_sweep}"
    )
    ...
    output_path = "tests/analog/bench_xxz_transverse_unitary_ensemble_autocorrelator.png"
    best_row = min(sweep_rows, key=lambda row: row[0])
    plot_ok = _save_plot(
        best_row[1], best_row[2], best_row[3], center_site, num_states, output_path, model_label=model_label
    )
    if plot_ok:
    ...
    ed_plot_output = "tests/analog/bench_xxz_transverse_unitary_ensemble_autocorrelator_ed.png"
    ed_plot_ok = _save_plot(
        reference_times,
        ed_mean_corr,
        ed_stderr_corr,
        center_site,
    ...
    combined_output = "tests/analog/bench_xxz_transverse_unitary_ensemble_vs_ed.png"
    ...
    error_output = "tests/analog/bench_xxz_transverse_unitary_ensemble_error_vs_ed.png"
